...
public class Airport {
	...
	public void createFlights(int size) throws ParseException {
		for (int i = 0; i < size; i++) {
			String hours = String.valueOf((int)Math.random()*24);
			String minutes = String.valueOf((int)Math.random()*60);
			Flight fg = new Flight(hours, minutes , airlines.get((int) Math.floor(Math.random() * airlines.size())), generateFlightsNumber(),
	...
	// binary search
	public void searchBYTime(String hours) throws ParseException {
		orderByTime();
		int init=0;
		boolean ended = false;
		int end= flights.size()-1;
		Flight toSearch = new Flight(hours, "", "", "", "", "", 1);
